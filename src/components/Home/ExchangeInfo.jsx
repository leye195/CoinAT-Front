...
const ExchangeInfo = ({coinInfo = [],upbitBitKrw, fixList = []}) => {
  ...
  return (
    coinInfo
  ?.filter((ticker) => ticker.symbol !== "BTC"&&!fixList.includes(ticker.symbol))
  .map((v) => {
        ...
        <Coin>
          <div className='star__wrapper'>
            <button type='button' onClick={handleWatchList(v.symbol)}>
              {!!watchList.find((symbol)=>symbol===v.symbol)?<AiFillStar className='watch'/>:<AiOutlineStar className='unwatch'/>} 
            </button>
          </div>
